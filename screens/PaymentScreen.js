// ---------------------------
// PaymentScreen.js (UPDATED with Order History)
// ---------------------------
...
export default function PaymentScreen({ navigation }) {
  ...
  const handleSubscribe = async () => {
    try {
      ...
      const premiumEntitlement =
        mockCustomerInfo.entitlements.active["magazine_quarterly"];
      if (!premiumEntitlement) {
        Alert.alert("Error", "Mock entitlement failed.");
        return;
      }
      const expiry = premiumEntitlement.expirationDate;
      const receipt = JSON.stringify(mockCustomerInfo);
      const level_id = 2;
      const token = await AsyncStorage.getItem("userToken");
      if (!token) {
      ...
      const response = await fetch(
        "https://contemporaryworld.ipcr.gov.ng/wp-json/ipcr/v1/update-subscription",
        {
          ...
          body: JSON.stringify({ level_id, expiry, receipt, mock: true }),
        }
      );
  ...
  if (
    String(membership?.level_id) === "2" ||
    String(membership?.membership_level) === "2"
  ) {
  ...
  return (
    <View style={styles.wrapper}>
      ...
      <Text style={styles.note}>Purchases are secure via App Store / Play Store.</Text>
